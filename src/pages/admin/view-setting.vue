<template>
	<div>
		...
		>
			<template #form-title> 显示信息 </template>
			<template #form-sub-title> 此页面显示网站前台的显示相关的信息 </template>
			...
			<z-form-item label="默认保存输入的文件夹密码">
				...
				<template #tips>
					可设置是否默认保存输入的文件夹密码, 如果为 true,
					则下次进入文件夹时会自动使用上次输入的密码，无需再次输入.
				</template>
			</z-form-item>
			...
			<z-form-item label="默认最大显示文件数">
				...
				<template #tips>
					默认最大显示文件数,
					用于控制文件夹中文件的显示数量，防止文件过多导致页面卡顿
				</template>
			</z-form-item>

			<z-form-item label="每次加载更多文件数">
				<el-input-number v-model="data.loadMoreSize" :min="1" />
				<template #tips> 当想显示更多文件时，每次额外显示的文件数 </template>
			</z-form-item>

			<z-form-item label="默认排序字段">
				<el-radio v-model="data.defaultSortField" label="name">文件名</el-radio>
				<el-radio v-model="data.defaultSortField" label="size"
					>文件大小</el-radio
				>
				<el-radio v-model="data.defaultSortField" label="time"
					>修改时间</el-radio
				>
				<template #tips>
			...
			<z-form-item label="OnlyOffice 服务地址">
				...
				<template #tips>
					<div>
						OnlyOffice 服务地址，默认的公共服务不保证稳定性，推荐自行部署
						OnlyOffice 服务. 部署教程：<a
							class="link"
							href="https://docs.zfile.vip/advanced#only-office"
							target="_blank"
							>https://docs.zfile.vip/advanced#only-office</a
						>
					</div>
					<div>
						提示：进行预览的文件需 OnlyOffice 服务器可访问，如您的 OnlyOffice
						在公网，要预览的文件在内网，则无法正常预览。
					</div>
					<div>
						提示：根据浏览器安全规范，如您的 ZFile 是 https 协议的，OnlyOffice
						服务也必须是 https 协议的，否则无法正常使用。
					</div>
				</template>
			</z-form-item>
